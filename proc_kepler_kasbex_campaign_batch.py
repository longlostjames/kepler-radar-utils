#!/usr/bin/env python3
"""
proc_kepler_kasbex_campaign_batch.py

Batch process KASBEX campaign data over a date range using campaign_processing module.

Usage:
    python proc_kepler_kasbex_campaign_batch.py --start-date YYYYMMDD --end-date YYYYMMDD
    python proc_kepler_kasbex_campaign_batch.py -d YYYYMMDD
    python proc_kepler_kasbex_campaign_batch.py -d YYYYMMDD --latest
...
def setup_kasbex_paths(use_latest=False):
    """Set up file and directory paths for KASBEX campaign."""
    home_path = Path.home()
    
    # Base KASBEX campaign paths
    base_inpath = '/gws/pw/j07/ncas_obs_vol2/cao/raw_data/ncas-mobile-ka-band-radar-1/data/campaign/kasbex/mom'
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Batch process KASBEX campaign radar data over a date range",
    ...
    try:
        ...
        print(f"KASBEX Campaign Processing")
        if args.latest:
    ...
    try:
        ...
        # Use default values
        campaign_info = {
            'tracking_tag': 'AMOF_KASBEX',
            'location': 'cao',  # or update with KASBEX location
    ...
    for i, datestr in enumerate(date_list, 1):
        print(f"\n{'='*60}")
        print(f"Processing KASBEX date {i}/{len(date_list)}: {datestr}")
        if args.latest:
        ...
        # Process the date
        try:
            print(f"Starting KASBEX processing for {datestr}...")
            ...
            print(f"✓ Successfully completed KASBEX processing for {datestr}")
    ...
    print(f"\n{'='*60}")
    print("KASBEX BATCH PROCESSING SUMMARY")
    print(f"{'='*60}")
    if args.date:
