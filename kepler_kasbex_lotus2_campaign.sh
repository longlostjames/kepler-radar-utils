...
#SBATCH --job-name=kasbex-campaign
...
PYTHON_SCRIPT="$SCRIPT_DIR/proc_kepler_kasbex_campaign_batch.py"

# Set date range (can be overridden via environment variables)
START_DATE=${START_DATE:-20250701}
END_DATE=${END_DATE:-20250930}
...
# Process the date
time python $PYTHON_SCRIPT -d ${DATESTR} --skip-missing --single-sweep --gzip --data-version 1.0.0
