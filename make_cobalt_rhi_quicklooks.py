...
def make_cobalt_rhi_plot(ncfile,figpath,blflag=False):
    ...
    nearest_file = find_nearest_file(cobalt_cmd_path, netcdf_time)
...
#            display.plot_colorbar(ax=ax[0],shrink=0.8);
        ...
        orig_title = ax[0].get_title();
        title_lines = orig_title.split("\n");

        target_elev = np.radians(float(cmd_data['elevation']));
        target_range = float(cmd_data['range'])/1000.0;

        target_height = target_range * np.sin(target_elev);


        ax[0].plot(target_range, target_height, 'b+')  


        target_str = f"{cmd_data['flight_id']} {cmd_data['icao_id']} {cmd_data['aircraft_type']} lean_burn={cmd_data['lean_burn']}"
        ...
        ax[2].set_title(new_title)
        ax[3].set_title(new_title)
...
def make_cobalt_vpt_plot_day(datestr,inpath,figpath,blflag=False):
    ...
    figname = f'ncas-radar-mobile-ka-band-1_chilbolton_cobalt_{datestr}_vpt_l1_{product_version}.png';
