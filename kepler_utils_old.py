...
def cfradial_add_ncas_metadata_v0(cfradfile,yaml_project_file,yaml_instrument_file,tracking_tag,data_version):
    ...
    DS.processing_software_url = "https://github.com/longlostjames/kepler-radar-utils/releases/tag/v1.0.0";
...
def cfradial_add_processing_software_metadata(cfradfile):
    # -------------------------------------------------------
    # Read cfradial file to add NCAS metadata
    # -------------------------------------------------------
    DS = nc4.Dataset(cfradfile,'r+');
        ...
        for file in files:
            ...
            if gzip_flag:
                if sweep_type in file and file.endswith('.mmclx.gz'):
                    ...
                    with gzip.open(fullfile) as gz:
                        with nc4.Dataset('dummy', mode='r', memory=gz.read()) as nc:
                            time_dim = nc.dimensions.get('time', None)
                            if len(time_dim)>0:
                ...
                if sweep_type in file and file.endswith('.mmclx'):
                    ...
                    time_dim = nc_file.dimensions.get('time',None);
                    if len(time_dim)>0:
            ...
            if gzip_flag:
                    ...
                    with gzip.open(fullfile) as gz:
                        with nc4.Dataset('dummy', mode='r', memory=gz.read()) as nc:
                            ...
                            if nrays>0:
                                ...
                                azim = (nc['azi'][i4fixed_angle]+revised_northangle) % 360;
                ...
                if "rhi" in file and file.endswith('.mmclx'):
                    ...
                    if nrays>0:
                        ...
                        file_time = cftime.num2pydate(nc['time'][0],'seconds since 1970-01-01 00:00:00')
                        #azim = (nc['azi'][0]+nc['northangle'][0]+azimuth_offset) % 360;
                        azim = (nc['azi'][3]+revised_northangle) % 360;
...
def multi_mmclx2cfrad(
    ...
    for i in range(1, len(files)):
        ...
        nrays = len(newRadarDS.time['data']);
...
def process_kepler_cobalt_day_step1(datestr,indir,outdir,yaml_project_file,yaml_instrument_file,azimuth_offset=0.0,gzip_flag=True,revised_northangle=55.7,data_version="1.0.0"):
    ...
    rhi_files = find_mmclx_rhi_files(start_date.strftime('%Y-%m-%d %H:%M:%S'), end_date.strftime('%Y-%m-%d %H:%M:%S'), 0, 370, indir,gzip_flag=True, azimuth_offset=azimuth_offset,revised_northangle=revised_northangle)
    ...
    if (len(rhi_files)>0):
        for f in rhi_files:
            #if gzip_flag:
            RadarDS_RHI = multi_mmclx2cfrad([f],outdir,scan_name='RHI',gzip_flag=True,azimuth_offset=azimuth_offset,tracking_tag='AMOF_20231120125118',campaign='cobalt',revised_northangle=revised_northangle,data_version=data_version);
            #else:
            #    RadarDS_RHI = multi_mmclx2cfrad([f],outdir,scan_name='RHI',gzip_flag=False,azimuth_offset=azimuth_offset,tracking_tag='AMOF_20231120125118',campaign='cobalt',revised_northangle=revised_northangle,data_version=data_version);

    
    # Vertically pointing files for whole day
    try:
        vpt_files = find_mmclxfiles(start_date.strftime('%Y-%m-%d %H:%M:%S'),end_date.strftime('%Y-%m-%d %H:%M:%S'),'vert', indir,gzip_flag=gzip_flag);
        print(vpt_files)
        #vpt_files_unzipped = find_mmclxfiles(start_date.strftime('%Y-%m-%d %H:%M:%S'),end_date.strftime('%Y-%m-%d %H:%M:%S'),'vert', indir,gzip_flag=False);
        if (len(vpt_files)>0):
            RadarDS_VPT = multi_mmclx2cfrad(vpt_files,outdir,scan_name='VPT',gzip_flag=gzip_flag,azimuth_offset=azimuth_offset,tracking_tag='AMOF_20231120125118',campaign='cobalt',revised_northangle=revised_northangle,data_version=data_version);
        #elif (len(vpt_files_unzipped)>0):
        #    RadarDS_VPT = multi_mmclx2cfrad(vpt_files_unzipped,outdir,scan_name='VPT',gzip_flag=False,azimuth_offset=azimuth_offset,tracking_tag='AMOF_20230201132601',campaign='ccrest-m',revised_northangle=55.7);
    except:
        print("VPT problem")
    ...
    try:
        vad_files = find_mmclx_vad_files(start_date.strftime('%Y-%m-%d %H:%M:%S'),end_date.strftime('%Y-%m-%d %H:%M:%S'),80,90, indir,gzip_flag=True);
        if (len(vad_files)>0):
            RadarDS_VAD = multi_mmclx2cfrad(vad_files,outdir,scan_name='VAD',gzip_flag=True,azimuth_offset=azimuth_offset,tracking_tag='AMOF_20231120125118',campaign='cobalt',revised_northangle=revised_northangle,data_version=data_version);
    except:
        pass

def process_kepler_cobalt_day_step1_latest(datestr,indir,outdir,yaml_project_file,yaml_instrument_file,azimuth_offset=0.0,gzip_flag=True,revised_northangle=55.7,data_version="1.0.0"):
    ...
    rhi_files = find_mmclx_rhi_files(start_date.strftime('%Y-%m-%d %H:%M:%S'), end_date.strftime('%Y-%m-%d %H:%M:%S'), 0, 360, indir,gzip_flag=True, azimuth_offset=azimuth_offset,revised_northangle=revised_northangle)
...
def process_kepler_kasbex_day_step1(datestr,indir,outdir,yaml_project_file,yaml_instrument_file,azimuth_offset=0.0,gzip_flag=True,revised_northangle=55.7,data_version="1.0.0"):
    ...
    rhi_files = find_mmclx_rhi_files(start_date.strftime('%Y-%m-%d %H:%M:%S'), end_date.strftime('%Y-%m-%d %H:%M:%S'), 0, 370, indir,gzip_flag=True, azimuth_offset=azimuth_offset,revised_northangle=revised_northangle)
    ppi_files = find_mmclx_ppi_files(start_date.strftime('%Y-%m-%d %H:%M:%S'), end_date.strftime('%Y-%m-%d %H:%M:%S'), 0, 370, indir,gzip_flag=True)


    print(f"rhi_files = {rhi_files}");
    print(len(rhi_files));
    print(f"ppi_files = {ppi_files}");
    print(len(ppi_files));

    if (len(rhi_files)>0):
        for f in rhi_files:
            RadarDS_RHI = multi_mmclx2cfrad([f],outdir,scan_name='RHI',gzip_flag=True,azimuth_offset=azimuth_offset,tracking_tag='AMOF_20250508133639',campaign='kasbex',revised_northangle=revised_northangle,data_version=data_version);

    if (len(ppi_files)>0):
        for f in ppi_files:
            RadarDS_PPI = multi_mmclx2cfrad([f],outdir,scan_name='PPI',gzip_flag=True,azimuth_offset=azimuth_offset,tracking_tag='AMOF_20250508133639',campaign='kasbex',revised_northangle=revised_northangle,data_version=data_version);
    ...
    try:
        ...
        print(vpt_files)
        if (len(vpt_files)>0):
            RadarDS_VPT = multi_mmclx2cfrad(vpt_files,outdir,scan_name='VPT',gzip_flag=gzip_flag,azimuth_offset=azimuth_offset,tracking_tag='AMOF_20250508133639',campaign='kasbex',revised_northangle=revised_northangle,data_version=data_version);
    except:
        print("VPT problem")
        pass
    # VAD files for whole day
    ...
    try:
        vad_files = find_mmclx_vad_files(start_date.strftime('%Y-%m-%d %H:%M:%S'),end_date.strftime('%Y-%m-%d %H:%M:%S'),80,90, indir,gzip_flag=True);
        if (len(vad_files)>0):
            RadarDS_VAD = multi_mmclx2cfrad(vad_files,outdir,scan_name='VAD',gzip_flag=True,azimuth_offset=azimuth_offset,tracking_tag='AMOF_20250508133639',campaign='kasbex',revised_northangle=revised_northangle,data_version=data_version);
    except:
        pass
