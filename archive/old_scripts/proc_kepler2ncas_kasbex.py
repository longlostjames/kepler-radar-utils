...
keplerpath = '/gws/pw/j07/ncas_obs_vol2/cao/raw_data/ncas-mobile-ka-band-radar-1';
ncas_radar_path = '/gws/nopw/j04/ncas_radar_vol2';
#keplerpath = os.path.join(ncas_radar_path,'cjw','projects');
amof_proc_path = '/gws/pw/j07/ncas_obs_vol2/cao/processing/ncas-mobile-ka-band-radar-1'
inpath = os.path.join(keplerpath,'data','campaign',campaign,'mom',yearstr,datestr);

if latest:
    inpath = os.path.join(keplerpath,'data','campaign',campaign,'mom',datestr,'latest');
    gzip_flag = False
else:
...
kepler_utils_old.process_kepler_kasbex_day_step1(datestr,inpath,outpath,yaml_project_file,yaml_instrument_file,azimuth_offset=azimuth_offset,revised_northangle=55.7,gzip_flag=gzip_flag,data_version=data_version);
