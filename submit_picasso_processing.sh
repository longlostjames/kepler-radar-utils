#!/bin/bash
#
# Script to submit PICASSO processing jobs to SLURM
# Run this from sci-vm-05 or another JASMIN sci VM
#

echo "PICASSO Campaign Data Processing Submission"
echo "============================================"
echo ""
...
case $choice in
    1)
        ...
        if [ -f "test_picasso_single.sh" ]; then
            sbatch test_picasso_single.sh
        else
            echo "test_picasso_single.sh not found. Create it first."
            exit 1
        fi
        ;;
    2)
        echo "Submitting array job for first 10 dates..."
        sbatch --array=1-10 kepler_picasso_lotus2_campaign.sh
        ;;
    3)
        ...
        if [[ $confirm == [yY] ]]; then
            sbatch --array=1-$TOTAL_DAYS kepler_picasso_lotus2_campaign.sh
        else
            echo "Cancelled"
        fi
        ;;
    4)
        ...
        sbatch --array=$array_range kepler_picasso_lotus2_campaign.sh
        ;;
    5)
...
")
        ...
        if [[ $confirm == [yY] ]]; then
            START_DATE=$custom_start END_DATE=$custom_end sbatch --array=1-$custom_days kepler_picasso_lotus2_campaign.sh
        else
            echo "Cancelled"
        fi
        ;;
